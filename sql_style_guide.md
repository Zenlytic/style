...
#### Name single-column primary keys `<table_name>_id`.
This allows the id to match perfectly with foreign keys and removes ambiguity in the event the table name is not obvious (especially in a BI or data app context).
...
#### Boolean column names:
  ...
  - Boolean columns should also always affirm something, instead of negate something. This is easier to comprehend and avoids a double negative in a where clause
...
If code containing commas needs to be split across multiple lines, put the commas at the end of the previous line.
  - This makes it easier to read.
  - Aesthetics matter and leading commas do not cut it visually
...
#### `from` clause:
  ...
  - If there are other tables being joined:
    ...
    - If there are multiple join conditions or the join condition is long, end the `join` line with the table name and put each condition on its own line (including the first one), indented one level more than the `join`.
...
from customers
left join orders
    on customers.customer_id_which_is_long = orders.customer_id
...
* Again, this just looks bad and looks matter.
```sql
/* Bad */
select *
from customers
...
This style guide was inspired in part by:
 ...
 - [Brooklyn Data Co's SQL style guide](https://github.com/brooklyn-data/co/blob/main/sql_style_guide.md)
